...
#@doc DOC_BFD1_exp_exp_exp
 function BFD1_exp_exp_exp(C_f, L_f,Ld_f, H::D, tmax::D, initT_f,BC_up_f,BC_dwn_f, M::Int, N::Int;
                 upper_bc_type::AbstractBoundaryCondition = DirichletBC() , 
                 lower_bc_type::AbstractBoundaryCondition = NeumanBC()) where D

   #x = range(0,H,N)# сетка по координате
    #t = range(0,tmax,M)# сетка по времени
    dx = H/(N - 1)
    dt = tmax/(M - 1)
    T = Matrix{D}(undef,N,M)# columns - distribution, rows time
    T1 = @view T[:,1]
    @. T1 = initT_f(0.0 : dx : H)
    ...
    Fm =    Vector{D}(undef,N)
    phi_m = Vector{D}(undef,N)
    lam_m = Vector{D}(undef,N)
