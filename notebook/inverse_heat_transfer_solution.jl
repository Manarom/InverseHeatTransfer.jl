...
solver_path = joinpath(source_path,"solvers")

# ╔═╡ a91ae0d5-6a90-472e-abd7-a5b02cf470fe
polynomials_path = joinpath(source_path,"polynomials")
...
# ╔═╡ 9f17647e-88f9-4451-bd4b-e638b190e2e8
IP = Main.InverseHeatTransfer

# ╔═╡ fa82cbba-1297-468b-ac9e-900f0de47ac4
Cp_fun

# ╔═╡ b7f738ad-2faa-4399-be33-71c43184a4c7
begin 
	C = IP.OptimizableVariable(IP.ScaledPolynomial(IP.BernsteinSymPoly(Cp_fun.poly.coeffs), xmin = lam_T_range[1], xmax = lam_T_range[end]))
	λ_poly = IP.ScaledPolynomial(IP.BernsteinSymPoly((17.0,10.0,8.0, 7.0)), xmin = lam_T_range[1], xmax = lam_T_range[end])
	λ = IP.OptimizableVariable(λ_poly, flag = false)
	dλdT_poly =  IP.PolynomialWrappers.derivative(λ_poly)
	dλdT = IP.OptimizableVariable(dλdT_poly, flag = false)
...
# ╔═╡ e99c2167-436f-4f2a-a8a6-abe2ef4e0225
dλdT.p.poly.coeffs

# ╔═╡ 2b18f4d0-b0f2-4169-a852-430cb6978b97
v1 = view([1,2,3,4], 2:3)

# ╔═╡ 9cc3e793-2dd3-4f64-a276-ae86b2c4a6b8
v1[1]

# ╔═╡ 8466b33d-91f5-4479-bf22-c835f42851b1
dλdT
...
possible_location_indices = 1:Main.OneDHeatTransfer.xpoints(problem.grid);
...
@bind  direct_problem_inds PlutoUI.combine() do Child
	...
	i4 = $(
		Child(Select(possible_location_indices,default=40))
	)\
	"""
end

# ╔═╡ 99f51ecf-36e4-4a0b-aa14-0757c88c69a3
md" time region for measured data [0.0 ... $(@bind measured_tmax Slider(Main.OneDHeatTransfer.trange(problem.grid), default = maximum(Main.OneDHeatTransfer.trange(problem.grid)), show_value = true) )]"

# ╔═╡ c1bd739f-39b0-4849-a468-cb177dab7d6a
begin 
	inds_real = unique(collect(direct_problem_inds))
	t_measured = collect(Main.OneDHeatTransfer.trange(problem.grid))
	flag = t_measured .< measured_tmax
	t_measured = t_measured[flag]
	measured_temperatures = transpose(problem.T[inds_real, flag])
	p_measured = Plots.plot(grid = true,gridlinewidth=3,gridstyle = :dot,minorgrid=true)
	for (i,c) in enumerate(eachcol(measured_temperatures))
		x_val = Main.OneDHeatTransfer.xvalue(problem.grid, inds_real[i])
...
# ╔═╡ 9c52036a-8b8e-4474-800b-56144d96a89c
typeof(measured_temperatures)
...
begin 
	...
	therm_locations = (i -> Main.OneDHeatTransfer.xvalue(problem.grid, i)).(inds_real)	
end
...
begin
	xpoints_number = 100
	tpoints_number = 2000
	inv_probl = IP.SingleInverseProblem(time_data, temperatures, initial_distribution, therm_locations, C,λ, dλdT, 15e-3, xpoints_number, tpoints_number)
end

# ╔═╡ 647ffae3-7945-437b-a551-ace4024638ad
IP.OneDHeatTransfer.solve_problem!(inv_probl.direct_problem)

# ╔═╡ 6f537a4c-bd46-465f-8846-e87a3f7d38c1
function plot_optimizable(C)

	return Plots.plot(range(C.p.xmax, C.p.xmin,100),C.p.(range(C.p.xmax, C.p.xmin,100)))
end

# ╔═╡ a302366d-27f9-4600-930d-fc0acc9456cd
begin 
	IP.refill!(λ, lam_changing_pars)
	dλdT_dummy_poly =  IP.PolynomialWrappers.derivative(λ.p)
	IP.refill!(dλdT, IP.PolynomialWrappers.coeffs(dλdT_dummy_poly) )
	IP.fill_residual!(inv_probl)
	discr  = norm(inv_probl.residual)
	p_fit_lam = plot_optimizable(λ)
	p_lam_inv = plot!(lam_T_range,lam_values)
	p_residual = Plots.plot(inv_probl.residual)
	discr
end

# ╔═╡ fad8260b-064e-40dd-a1ac-553b9760522a
p_lam_inv

# ╔═╡ 412d3c0a-867f-4ef5-acc4-fc593ad03e1d
p_residual
...
manifest_format = "2.0"
project_hash = "9bd87e192e451f7f1de40a4639185600bc0827a0"
...
git-tree-sha1 = "03dea6c46a9c9eb2d1242c6a33fb106e0c759ae9"
...
git-tree-sha1 = "9c83131d543f8275059baecbf98f8af43bd44783"
