...
import Pkg
	...
	import InverseHeatTransfer.OneDHeatTransfer as OHT
end
...
# ╔═╡ 7a052646-96c3-4deb-ae00-fad6f8465b17
...
begin 
	...
	Plots.plot(1e3*x_dummy,init_values;plot_common_args...,title="Starting temperature",label= nothing,linewidth = 4)
...
begin 
	...
	p_cp = Plots.plot(lam_T_range,cp_values./Ro;plot_common_args...,title="Specific heat",label= nothing,linewidth = 4)
...
begin 
	...
	p_lam = Plots.plot(lam_T_range,lam_values;plot_common_args...,title="thermal conductivity",label= nothing,linewidth = 4)
...
problem = OHT.HeatTransferProblem(Cp_fun, lam_fun,lam_der,initT_f, H,N, tmax, M, BC_up_f, BC_dwn_f, Float64, upper_bc_type(),lower_bc_type());
...
begin 
	...
	# lower BC plot
	p_lbc = Plots.plot(low_x_data,BC_dwn_f.(low_x_data);plot_common_args...,title="lower BC:"*string(lower_bc_type),label= nothing,linewidth = 4)
	xlabel!(p_lbc,low_x_data_name)
	ylabel!(p_lbc,"lower BC")
	# upper BC plot
	p_ubc = Plots.plot(up_x_data,BC_up_f.(up_x_data);plot_common_args...,title="upper BC:"*string(upper_bc_type),label= nothing,linewidth = 4)
...
begin 
	...
	if is_plot_coordinate
		...
		for x_cur in x_iterpolation
			bknd.plot!(ppp,time,Tinterp.(x_cur,time);plot_common_args..., label = string(round(1e3*x_cur,digits = 2)),linewidth = 3,linealpha = 0.8)
		end		
		xlabel!(ppp,"Time, s")
		ylabel!(ppp,"Temperature")
	end
...
if reference_eval
	reference_problem = OHT.HeatTransferProblem(Cp_fun, 					lam_fun,lam_der,initT_f, H,1500, tmax,50000,BC_up_f,BC_dwn_f,Float64,upper_bc_type(),lower_bc_type());
...
if (is_check_all && reference_eval) 
	...
	for j in 1 : sol_number # column  - solver type
		...
		plot!(p_stat_M, m_vect, abs.(quant2plot_vect);plot_common_args...,yscale = :log10, xscale = :log10, label = string(sol_names[j]),marker=true, markersize=8, markershape= :auto, linewidth = 3)
	end
...
if is_check_all && reference_eval && is_bench_all
	bm_stat_M = Plots.plot(;plot_common_args...,marker=true, markersize=8, markershape=:auto)
	...
	for j in 1 : size(stats,2) # column  - solver type
		...
		plot!(bm_stat_M, m_vect, bench2plot_vect, xscale = bm_xscale,yscale=bm_yscale;plot_common_args..., label = string(sol_names[j]),marker=true, markersize=8, markershape= :auto, linewidth = 3)
	end
...
if is_check_all && reference_eval && is_bench_all
		pretty_table(HTML,hcat(m_vect, dt_vect,bm_accuracy_matrix*1e-6),column_labels = table_columns, title = "$(bm_fun) $(bm_field) ,ms" , compact_printing = true)
end
